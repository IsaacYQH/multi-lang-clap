...
def tokenizer(text, tmodel="roberta", max_length=77):
    ...
    elif tmodel == "xlm-roberta":
        result = xlmRoberta_tokenizer(
            text,
            padding="max_length",
            truncation=True,
            max_length=max_length,
...
class ToyDataset(Dataset):
    def __init__(self, index_path, ipc, config, eval_mode=False):
        ...
        self.fp = h5py.File(index_path, "r") #同一个目录下放下所有需要训练的音频
        self.ipc = np.load(ipc, allow_pickle=True) #ipc是class信息的index，也包含每个音频在index_path里的index
    ...
    def generate_queue(self):
        ...
        while len(self.queue) < self.total_size:
            ...
            #随便挑一个加入queue
            self.queue += [
                self.ipc[d][random.randint(0, len(self.ipc[d]) - 1)] for d in class_set #ipc的第一层：class名： 第二层：里面对边选一个
            ]
...
def get_data(args, model_cfg):
    ...
    if args.train_data:
        data["train"] = get_dataset_fn(args.dataset_type)(
            args, model_cfg, is_train=True, audio_ext="wav"
        )

    if args.val_data:
        data["val"] = get_dataset_fn(args.dataset_type)(
            args, model_cfg, is_train=False, audio_ext="wav"
        )
