...
def main():
    ...
    # discover initial world args early so we can log properly: 与程序的并行处理和分布式环境相关
    ...
    if is_master(args):
        args.tensorboard_path = (
            os.path.join(args.logs_dir, args.name, "tensorboard")
            if args.tensorboard
            else ""
        )
        args.checkpoint_path = os.path.join(args.logs_dir, args.name, "checkpoints")
