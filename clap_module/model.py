...
from transformers import BertModel, RobertaModel, BartModel, XLMRobertaModel
...
class CLAP(nn.Module):
    ...
    def init_text_branch_parameters(self):
        ...
        if self.text_branch_type == "bert" or self.text_branch_type == "roberta" or self.text_branch_type == "xlm-roberta":
            width = self.text_branch.embeddings.word_embeddings.weight.shape[-1]
        elif self.text_branch_type == "bart":
            width = self.text_branch.shared.weight.shape[-1]
        # elif self.text_branch_type == "xlm-roberta": #fairseq package
        #     width = self.text_branch.model.args.encoder_embed_dim
        else:
    ...
    def encode_text(self, text, device):
        if self.text_branch_type == "transformer":
            ...
            x = self.text_projection(x)
        elif self.text_branch_type == "xlm-roberta":
            x = self.text_branch(
                input_ids=text["input_ids"].to(device=device, non_blocking=True),
                attention_mask=text["attention_mask"].to(
                    device=device, non_blocking=True
